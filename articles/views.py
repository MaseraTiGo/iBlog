...
class Index(View):
    ...
    def get(self, request):
        ...
        info = Info.objects.all().order_by('id')[0]
...
class ArticlesView(View):
    ...
    def get(self, request, pk):
        ...
        # 首页/footer 轮播文章
        banner_arts = articles.filter(is_index=1)
        if len(banner_arts) >= 6:  # 首页轮播默认选择5篇文章
            banner_arts = banner_arts[:6]
        else:  # 不足6篇文章，则使用最后一篇文章凑数
            banner_arts = list(banner_arts)
            for i in range(6 - len(banner_arts)):
...
class SearchView(View):
    ...
    def get(self, request):
        ...
        list_articles = articles.filter(Q(title__icontains=keyword) | Q(categorie__name__icontains=keyword))
        try:
            page = request.GET.get('page', 1)
        except PageNotAnInteger:
            page = 1
